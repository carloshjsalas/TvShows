...
import com.cs.tvshows.ui.details.DetailsActions
import com.cs.tvshows.ui.details.DetailsViewModel
...
open class DetailsViewModelTest {
    ...
    fun setup() {
        ...
        detailsViewModel = DetailsViewModel(tvShowsRepository)
        tvShow = TvShow(
            ID,
            VOTE_AVERAGE,
            TITLE,
    ...
    fun `getTvShowsFirstPage SHOULD return a list of tv shows after reach the repository`() =
        runTest {
            val response = Outcome.Success(tvShow)
            val channel = Channel<Outcome<TvShow>>()
            val flow = channel.consumeAsFlow()

            Mockito.`when`(tvShowsRepository.getTvShowDetails(TV_SHOW_ID)).thenReturn(flow)

            launch { channel.send(response) }

            detailsViewModel.getTvShow(TV_SHOW_ID)

            delay(DELAY)

            Assert.assertEquals(
                tvShow,
                (detailsViewModel.tvShowOutcome.value?.data as DetailsActions.OnTvShowLoaded).item
            )
        }
